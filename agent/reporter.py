...
def build_html_report(items: list[ClassifiedItem], run_date: str) -> str:
    ...
    def item_card(item: ClassifiedItem) -> str:
        ...
        action_flag = (
            '<div style="margin-top:6px;color:#b71c1c;font-weight:bold">[ACTION REQUIRED]</div>'
        ...
        return (
            ...
            f'<div style="font-size:13px;color:#757575;margin-bottom:4px">{item.source}</div>'
            f'<a href="{item.url}" style="font-size:16px;font-weight:bold;color:#1565c0;text-decoration:none">{item.title}</a>'
            f'<p style="margin:10px 0;color:#212121;line-height:1.6">{item.summary}</p>'
            f'<div style="font-size:13px">{impact_row}</div>'
            f'{action_flag}'
            '</div>'
        )
    ...
    for cat, cat_items in by_category.items():
        ...
        category_sections += (
            f'<h2 style="color:#1a237e;border-bottom:2px solid #3f51b5;padding-bottom:6px">{cat}</h2>'
            f'{cards}'
        )
